...
当前实现说明：本地 MVP 已实现 Source Registry、Collectors、SQLite storage、基础聚类、排序、Daily Brief、即时问答、反馈、SMTP 邮件、Windows 计划任务脚本、双版本简报、翻译和运行日志。Watchers、Breaking Alert、专用 Deep Dive、完整 Citation Verifier、HTML 报告、retention cleanup 和源健康自动禁用仍是规划或部分设计。
...
规划模块，尚未在当前代码中实现。目标是监控重大事件并触发 Breaking Alert。
...
当前负责生成每日简报、问答回答和末端翻译。重大提醒和专用 Deep Dive 属于后续规划。
...
规划中的完整校验模块。当前代码已对部分输出做轻量约束，例如保护 URL 和 Story ID、校验问答来源 URL、清理模型输出，但还没有独立的完整 Citation Verifier。
...
4. 通过 Windows Task Scheduler 脚本支持本地定时运行。
5. 实现主要 Collectors：市场行情、RSS/Atom、CCTV 页面、Hugging Face、GitHub、AI 硬件新闻。
6. 实现基础 Normalizer、Clusterer 和 Ranker。
7. 实现 Daily Brief、双版本简报和末端翻译。
8. 实现一个推送渠道：SMTP Email。
9. 实现即时问答。
10. 实现基础反馈机制。
11. 增加 pipeline、source collection、delivery 和 LLM run 日志。

后续：

1. 改进多来源聚类和 evidence 合并。
2. 增加完整 Citation Verifier。
3. 增加 retention cleanup。
4. 增加 source health 和自动禁用策略。
5. 增加 HTML 报告。
6. 决定 Breaking Alert 和 Deep Dive 是否进入下一阶段。
